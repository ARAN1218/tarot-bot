...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const prompt = `
            ...
            3. そのために使用するタロット占い方法は「ワンオラクル」です。1枚のタロットカードを引き、その意味から質問について占う方法です。
            ...
            はい、引かれました。「${TAROT[card[0]].カード名}」です。

            このカードは「${TAROT[card[0]].意味}」を意味します。

            これは、${question}の(カードに対応すると考えられる要素)と結びついています。

            (質問に応じて1枚目のカードを何かに当てはめて解釈し、占う)

            (また、もしあればカードに込められた別の意味に基づいて、追加で解釈し、占う)
