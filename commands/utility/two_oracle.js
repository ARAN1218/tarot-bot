...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const prompt = `
            ...
            3. そのために使用するタロット占い方法は「ツーオラクル」です。ツーオラクルは2枚のカードから質問について占います。カードは質問の何かに対応しています。例えば「結果/対策」、「A択/B択」等です。これは質問に応じて当てはめて考えてください。
            4. 今デッキがシャッフルされ、引かれたカードは「${TAROT[card[0]].カード名}」と「${TAROT[card[1]].カード名}」でした。そのカードの意味はそれぞれ「${TAROT[card[0]].意味}」と「${TAROT[card[1]].意味}」でした。カードが何を表しているかは質問内容から判断してください。
            ...
            はい、引かれました。「${TAROT[card[0]].カード名}」「${TAROT[card[1]].カード名}」です。
